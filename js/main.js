$(document).ready(function() {
    ...
    $("#currentDate").append('<span class="currentDate">' + "Current Time: " + currentDate + '</span>');
    ...
    var needsResponse = function(email){
    //An email will require a response if the difference between the current time and the time
    //the email was received exceeds the time the sender predicted you to respond. 
    //If this is the case then the email in the inbox will be red.
    	//alert("SENDER: "+sender.dateSent);
    	var date = new Date(email.dateSent);
    	var diff = (currentDate - date)/1000;
    	
    	console.log("Email Date: " + date);
    	console.log("DIFFERENCE: " + diff);
    	if(diff < 0) console.log("ERROR: Time in email is greater than 'Current Time'!");
		var predTimeInSec = 0;
		//times [0] is number, times[1] is day, days, hr, min, or mo
    	var times = email.myPredictedTime.split(" ");
    	//turns predicated time into seconds, so we can compare the predicated time
    	//to how much time has passed since the user received the email. 
		
    	switch(times[1]){
    		case "days":
    			predTimeinSec = times[0]*24*60*60;
    			break;
    		case "day":
    			predTimeinSec = times[0]*24*60*60;
    			break;
    		case "hr":
    			predTimeinSec = times[0]*60*60;
    			break;
    		case "mo":
    			predTimeinSec = times[0]*30*24*60*60;
    			break;
    		case "min":
    			predTimeinSec = times[0]*60;
    			break;
    	}
    	//alert("PREDTIMEINSEC for  " + times[1] + "  " + predTimeinSec);
    	if (diff > predTimeinSec){
    		return true;
    	}else{
    		return false;
    	}
	...
	var formattedDate = function(email) {
    	...
    	return ("Received: " + monthNames[date.getMonth()] + " " + date.getDate() + ", " + date.getHours() + ":"+minutes);
	}
    ...
    var append_emails = function(emails) {
        ...
        for (var i = 0; i < emails.length; i++) {
            ...
            if(interface_select=="time"||interface_select=="timeNparen"){
          	 	 if(sentOpen){
           		 	$('#email'+i).append('<span class="date">' + formattedDate(email) +"&nbsp;&nbsp;&nbsp;&nbsp;"+"Response In: "+ email.myPredictedTime + '</span>');
        			}else{
        			$('#email'+i).append('<span class="date">' + formattedDate(email) +"&nbsp;&nbsp;&nbsp;&nbsp;"+"Respond Within: "+ email.myPredictedTime + '</span>');
        		}
        	}else{
        		$('#email'+i).append('<span class="date">' + formattedDate(email) + '</span>');
        	}
        	if(interface_select=="time"||interface_select=="timeNparen"){
            		if(needsResponse(email)){
...
//          $('#email'+i).append('<span class="date">' + formattedDate(email) + '</span>');
	...
	$('#nav_contacts').click(function() {
		...
		if (email_editor_open)
			...
			$('#email_list').attr('class','col-md-8')
	...
	//Click functions for reply and forward buttons.
	$('#reply').click(function() {
        ...
        var  content = $("#expanded_sender.tracked").text();
        //if content is not already in the subject line, wrap the content
        if(content != $(".tracked.click.to_field_contact_name").text()) wrap_contact(content);
        //Sets subject line to RE
        if($("#expanded_subject").text().substring(0,3) == "RE:"){
            $('#subect_field.form-control.tracked.text').val($("#expanded_subject").text());
        } else{
            $('#subect_field.form-control.tracked.text').val("RE: "+$("#expanded_subject").text());
        }
    ...
    $('#forward').click(function() {
        ...
        $('#email_list').attr('class','col-md-4');
        $('#new_message_editor').show();
        //After clicking forward, it adds message and Fwd: to body and subject line, respectively.
        $('#subect_field.form-control.tracked.text').val("Fwd: "+$("#expanded_subject").text());
        $('#message_field').val("Begin Forwarded Message:" +"\n"+"From: "+ $("#expanded_sender").text()+ "\n" +"\n"+
            "-------------------------------------------------------------------------------"+"\n"+"\n"+$("#expanded_content").text())
        
    });

    $('#advanceTime').click(function(){
    ...
    var wrap_contact = function(content) {
         ...
         //for loop checks if any of the same contact you are adding is already in the tofield.
         //if it is, email is not added.
